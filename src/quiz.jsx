...
export default function Quiz() {
  const [count, setCount] = useState(0) //used to count how many answers the user got right
  const [result, setResult] = useState(+false) //keeps track of whether the quiz has been submitted
  const [questions, setQuestions] = useState([]) //used to store the fetched questions
  const [loading, setLoading] = useState(false) //used to enable or disable the form's submit button while the quiz is being reset
  const [fetching, setFetching] = useState(true) //used to show or hide the form's submit button while the quiz is being initialized for the first time
  const [showQuizError, setShowQuizError] = useState(false) //keeps track of whether the error message prompting the user to select all answers should be displayed
  const [errorAPI, setErrorAPI] = useState(null) //stores the error message in case of an error occurring while fetching questions from the OpenTrivia API
  ...
  async function handleSubmit(event) {
    ...
    if (!result) {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (
          questions[i].selectedAnswer === decode(questions[i].correct_answer)
        ) {
    ...
    <div>
      ...
      {fetching ? (
        ...
        <form onSubmit={handleSubmit}>
          <fieldset>
            <div className="question-section">{renderQuestions()}</div>
          </fieldset>

          {/* Conditional rendering of the user's quiz score */}
          {!!result && (
            <p className="quiz-result">
              You scored {count}/{questions.length} correct answers
            </p>
          )}

          {/* Conditional rendering of error message prompting user to select an answer for each question */}
          {!result && showQuizError && (
            <p className="error-message">Please answer all questions.</p>
          )}

          {/* Switches text of button depending on state of result and loading
                    Button is disabled depending on the state of loading */}
          <button className="quiz-button" disabled={loading}>
            {loading ? "Resetting..." : result ? "Play again" : "Check answers"}
          </button>
        </form>
      )}
    </div>
